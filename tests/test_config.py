...
def test_session_timeout_settings_have_safe_defaults():
    ...
    assert settings.demo_token_secret == ""
    assert settings.demo_token_ttl_seconds == 300
...
def test_session_timeout_settings_load_from_environment(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("DEMO_TOKEN_SECRET", "server-side-secret")
    monkeypatch.setenv("DEMO_TOKEN_TTL_SECONDS", "300")
    ...
    assert settings.demo_token_secret == "server-side-secret"
    assert settings.demo_token_ttl_seconds == 300
...
def test_session_timeout_settings_reject_invalid_values():
    ...
    with pytest.raises(ValueError):
        AppSettings(demo_token_ttl_seconds=0)
